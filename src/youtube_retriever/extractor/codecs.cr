module Youtube
  module Codecs
    ...
    TITLE = [:itag, :container, :video_resolution, :video_encoding, :video_profile, :video_bitrate, :audio_encoding, :audio_bitrate, :comment]
    CODECS_STR = String.build do |codecs|
      # Non-DASH
      codecs << <<-EOF
    ...
    CODECS = CODECS_STR.split("\n").map do |line|
      line = line[1..-2].split("|").map { |x| x.strip }
      line << case
        when line[6].empty?  then "video only"
        when line[2].empty?  then "audio only"
        when line[1] == "TS" then "live stream"
        else "default"
      end
      { line[0] => TITLE.zip(line).to_h }
